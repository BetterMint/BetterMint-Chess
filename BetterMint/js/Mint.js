...
class GameController {
  constructor(BetterMintmaster, chessboard) {
    ...
    this.controller.on('ModeChanged', (event) => {
      if (event.data === "playing") {
        // Delay slightly to let chess.com update the board state before we read it
        setTimeout(() => {
          this.ResetGame();
    ...
    this.controller.on("ResetGame", (event) => {
      // Defer slightly to let chess.com update the board visually first
      setTimeout(() => {
    ...
    setTimeout(() => {
        if(!checkEventOne){
            // Fallback - just for safety, no action needed
        }
    }, 1100);
    this.controller.onAll((event) => {
      // Debug: log all events to see what fires during new games
      if (event.type && (event.type.includes("Reset") || event.type.includes("Game") || event.type.includes("Mode") || event.type.includes("Renderer"))) {
        console.log("GameController Event:", event.type, event.data);
      }
    });
  ...
  setupAutoQueueObserver() {
    ...
    this.autoQueueObserver = new MutationObserver(() => {
      const now = Date.now();
      // Debounce: only check every 200ms to avoid excessive DOM queries
      ...
      const selectors = [
        ...
        'button[data-test-id="game-over-buttons-button"]',
      ...
      for (const selector of selectors) {
        ...
        for (const btn of buttons) {
          ...
          // Look for "new game" or just "new" button
          if (text.includes('new')) {
            console.log("Auto Queue: Clicking New Game button");
            btn.click();
            return;
          }
        }
      ...
      const intervalId = setInterval(() => {
        ...
        for (const selector of selectors) {
          ...
          for (const button of buttons) {
            ...
            // Look for "new game" or just "new" button
            if (text.includes('new')) {
              console.log("Auto Queue: Clicking New Game button from handleGameOver");
              button.click();
              clearInterval(intervalId);
              return;
            }
  ...
  ResetGame() {
    // Refresh options from the controller to get the correct player color for the new game
    this.options = this.controller.getOptions();
    console.log("ResetGame - Player Color Updated:", this.options.isPlayerBlack ? "Black" : "White");
    this.UpdateEngine(true);
    BetterMintmaster.game.RefreshEvalutionBar();
...
class StockfishEngine {
  constructor(BetterMintmaster) {
    let stockfishJsURL;
    let stockfishPathConfig = Config && Config.threadedEnginePaths ? Config.threadedEnginePaths.stockfish : null;
    this.BetterMintmaster = BetterMintmaster;
    this.loaded = false;
   ...
   "MultiPV": getValueConfig(enumOptions.MultiPV),
    ...
    if (!getValueConfig(enumOptions.ApiStockfish)) {
      if (stockfishPathConfig) {
        ...
        console.warn("Stockfish path config not found, falling back to WebSocket API");
            ...
            setTimeout(() => {
              ...
              if (getValueConfig(enumOptions.Notifications) && window.toaster) {
                window.toaster.add({
                  ...
                  duration: 2000,
                  icon: "circle-info",
                  message: `Pre-move ${this.moveCounter}/${getValueConfig(enumOptions.MaxPreMoves)} executed!`,
                  style: {
                    position: "fixed",
                    bottom: "120px",
                    right: "30px",
              ...
              if (this.moveCounter >= getValueConfig(enumOptions.MaxPreMoves)) {
                if (getValueConfig(enumOptions.Notifications) && window.toaster) {
                  window.toaster.add({
                    ...
                    message: "Maximum pre-moves reached!",
                    style: {
                      position: "fixed",
                      bottom: "120px",
                      right: "30px",
          ...
          if (moveData) {
            ...
            if (getValueConfig(enumOptions.Notifications) && window.toaster) {
              window.toaster.add({
                ...
                duration: 2000,
                icon: "circle-checkmark",
                message: `BetterMint: Mate in ${bestMove.mate} move(s)! Executing premove...`,
                style: {
                  position: "fixed",
                  bottom: "120px",
                  right: "30px",
    ...
    if (
      ...
      if (getValueConfig(enumOptions.Notifications) && window.toaster) {
        window.toaster.add({
          ...
          icon: "circle-info",
          message: `BetterMint: Auto move in ${secondsTillAutoMove} seconds`,
          style: {
            position: "fixed",
            bottom: "60px",
            right: "30px",
...
function InitBetterMint(chessboard) {
  // Fetch the ECO table
  if (Config && Config.pathToEcoJson) {
    fetch(Config.pathToEcoJson).then(function (response) {
      ...
      console.warn("Could not load ECO table:", error);
      eTable = null;
    });
  }
